...
def get_response(user_input):
    ...
    if "search" in processed_input and "for" in processed_input:
        try:
            ...
            query = " ".join(processed_input[for_index + 1:])
    ...
    if "what" in processed_input and "is" in processed_input:
        try:
            what_index = processed_input.index("what")
            is_index = processed_input.index("is")
            if is_index == what_index + 1:
                query = " ".join(processed_input[is_index + 1:])
